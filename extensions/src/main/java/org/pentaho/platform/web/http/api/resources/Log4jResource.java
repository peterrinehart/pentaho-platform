...
public class Log4jResource {
  ...
  public Response reloadConfiguration() throws Exception {
    setLogLevel( LOGGER.getName(), Level.INFO );
  ...
  public Response updateLogLevel( @FormParam(  "level" ) String targetLevel, @FormParam( "category" ) String category ) throws Exception {
    ...
    if ( StringUtils.isNotBlank( targetLevel ) ) {
      ...
      if ( StringUtils.isNotBlank( category ) ) {
        ...
        LoggerConfig loggerConfig = getLoggerConfig( category );
        if ( loggerConfig != null ) {
          setLogLevel( category, Level.toLevel( targetLevel, root.getLevel() ) );
          return Response.ok( "Setting log level for: '" + category + "' to be: " + targetLevel ).build();
        }
        return Response.notModified(
          "Category: '" + Encode.forHtml( category ) + "' not found, log level not modified." ).build();
      }

      setLogLevel( root.getName(), Level.toLevel( targetLevel, root.getLevel() ) );
      LOGGER.info( "Root logger level set to: " + root.getLevel() );

      LoggerContext ctx = (LoggerContext) LogManager.getContext( false );
      Configuration config = ctx.getConfiguration();
      for ( String loggerName : config.getLoggers().keySet() ) {
        setLogLevel( loggerName, Level.toLevel( targetLevel, root.getLevel() ) );
      }
    }
